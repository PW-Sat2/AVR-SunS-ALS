...
volatile uint8_t *TWISlaveRegisterAccess::tx_buffer_start = nullptr;
volatile uint8_t TWISlaveRegisterAccess::tx_buffer_cnt;
uint8_t TWISlaveRegisterAccess::tx_buffer_max = 0;
...
ISR(TWI_vect) {
    ...
    case TW_ST_SLA_ACK:
        TWISlaveRegisterAccess::tx_buffer = TWISlaveRegisterAccess::tx_buffer_start;
        TWISlaveRegisterAccess::tx_buffer_cnt = 0;
        [[fallthrough]];
    case TW_ST_DATA_ACK:
        if (TWISlaveRegisterAccess::tx_buffer_cnt < TWISlaveRegisterAccess::tx_buffer_max) {
            TWDR = *(TWISlaveRegisterAccess::tx_buffer++);
            TWISlaveRegisterAccess::tx_buffer_cnt++;
        }
        break;
